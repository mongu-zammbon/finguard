...
오늘 데모는 **보이스피싱/기관 사칭**으로 진행한다. 서비스 화면의 기본 진입은 `공유받은 문자`로 두고, `스크린샷`과 `결제 링크` 경로도 직접 보여준다. 의사결정 슬라이드에는 **중고거래 사기**를 명시적인 2차 wedge 후보로 올려 팀 투표를 받는다. 이는 금융 현안 임팩트와 초기 사용성 가설이 충돌하는 지점이며, 실제 공개 데이터·사용자 행동 실험으로 결정한다.

## 합의

1. 첫 MVP는 보이스피싱·기관 사칭·안전계좌 이체·원격제어 요청에 집중한다.
2. 공유/스크린샷/링크 → 라벨 → 증거 → 다음 행동의 4단계 UX를 90초 안에 보여준다.
...
- 피해 의심 사용자+가족/보호자와 상담원 중 첫 타깃은 오늘 팀 투표로 확정한다.
- 보이스피싱과 중고거래 사기 중 첫 wedge는 오늘 팀 투표 후 고정한다.
- Claude Bedrock은 오탐률, 공유 이탈률, 설명가능성 행동전환을 우선 검증하라고 제안했다.
